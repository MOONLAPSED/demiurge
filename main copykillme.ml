...
(* Helper to print replication chains *)
let print_replication_chain quine =
  Printf.printf "Quine #%d — Replications: %d | Score: %.2f\n"
    quine.id quine.replication_count quine.intensive_score;
  match quine.replication_trace with
  | Some trace when List.length trace > 1 ->
      Printf.printf "  Replication Lineage: ";
      List.iter (Printf.printf "#%d -> ") (List.rev trace);
      Printf.printf "✱\n"
  | _ -> Printf.printf "  [No trace available]\n"
(* Quine Demos - different self-replicating patterns *)
type quine_demos = 
...
(* Helper to prove self-amplifying computational complexity *)
let prove_unbounded_complexity computor =
  (* Select quines with high resourcefulness or intensity *)
  ...
  let base_pop = float_of_int computor.initial_population in
  let ratio =
    if base_pop = 0.0 then 0.0
    else float_of_int total_replications /. base_pop
  in

  (* Diagnostic Output *)
  Printf.printf "\n=== Complexity Emergence Report ===\n";
  Printf.printf "Phase: %s | Generation: %d\n" computor.phase computor.generation;
  Printf.printf "Active population: %d\n" (List.length computor.population);
  Printf.printf "Intensive quines: %d\n" num_intensive;
  Printf.printf "Replications (from intensive agents): %d\n" total_replications;
  if diversity_score >= 0 then
    Printf.printf "Species diversity: %d\n" diversity_score;
  Printf.printf "Amplification ratio: %.2f×\n" ratio;

  let verdict =
    if total_replications > 512 then
      "✓ Quine storm: uncontrolled replication — unbounded behavior verified."
    else if num_intensive >= 4 then
      "○ Active thermogenesis: replication clusters forming."
    else if num_intensive > 0 then
      "• Sparse quines detected — system not yet critical."
    else
      "× No intensive quines — entropy remains dominant."
  in
...
(* Entry point for simulation and analysis *)
let () =
  Random.init (int_of_float (Unix.time ()));

  Printf.printf "=== Thermodynamic Quine Engine ===\n";
  Printf.printf "Bit Encoding: <C|VVV|TTTT> — Control | Value | Thermodynamic Tags\n\n";

  let computor = Computor.init_simulation ~population_size:256 in

  (* Optional: perform warm-up or burn-in *)
  Computor.evolve_for computor ~generations:32;

  let survivors = prove_unbounded_complexity computor in

  (* Visualize and analyze survivors *)
  List.iter (fun q ->
    try demonstrate_intensive_character q
    with _ ->
      Printf.printf "[!] Visualization failed for quine ID %d\n" q.id
  ) survivors;

  (* Post-analysis *)
  Printf.printf "\n=== Final Remarks ===\n";
  Printf.printf "From an initial field of 256, emergent replicators arose.\n";
  Printf.printf "These entities exhibit computational vitality — adapting under entropy.\n";
  Printf.printf "Not all systems burn out; some burn *onward*.\n\n"
;;
