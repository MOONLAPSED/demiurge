...
let prove_unbounded_complexity computor =
  ...
  (* Visualize top replicators *)
  let sorted =
    List.sort (fun a b -> compare b.replication_count a.replication_count) intensive_quines
  in
  let top = List.take_opt 5 sorted |> Option.value ~default:[] in
  Printf.printf "\nTop replicators:\n";
  List.iter print_replication_chain top;
...
let () =
  ...
  Printf.printf "Initializing computor with 256 base organisms...\n\n";

  let computor = Computor.init_simulation ~population_size:256 in

  (* Run the simulation to steady state or threshold *)
  Printf.printf "Evolving computation...\n";
  let computor = Computor.run_until ~max_generations:1000 ~until:Computor.unbounded_activity computor in

  (* Attempt proof of unbounded complexity *)
  let survivors = prove_unbounded_complexity computor in

  (* Demonstrate intensive replicators *)
  Printf.printf "\nDemonstrating intensive replicators:\n";
  List.iter (fun q ->
    try
